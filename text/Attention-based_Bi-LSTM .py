...
def make_df(train_path, test_path, max_features, maxlen):
    train = pd.read_csv(train_path)
    test = pd.read_csv(test_path)
    train = train.sample(frac=1)

    list_sentences_train = train["title"].fillna("unknown").values
    y = train['Category'].values
    y_test = test['Category'].values
    list_sentences_test = test["title"].fillna("unknown").values
    y = to_categorical(y)
...
if __name__ == "__main__":
    ...
    xtr, xte, y, y_test,word_index = make_df("train.csv",
                                      "test_1.csv",
                                      max_features, maxlen)
    ...
    model.fit(xtr, y, batch_size=256, epochs=1, validation_split=0.1, callbacks=[ckpt, early])
    ...
    sample_submission['Category'] = y_test
